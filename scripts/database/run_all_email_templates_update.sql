-- تشغيل جميع ملفات تحديث قوالب الإيميلات
-- Run All Email Templates Update Files
-- تاريخ الإنشاء: 21 سبتمبر 2025

-- 1. إصلاح سياسات RLS أولاً
...
DROP POLICY IF EXISTS "Allow read access to email notification types" ON email_notification_types;
DROP POLICY IF EXISTS "Allow read access to email templates" ON email_templates;
DROP POLICY IF EXISTS "Allow read access to email settings" ON email_settings;
DROP POLICY IF EXISTS "Allow read access to email logs" ON email_logs;
DROP POLICY IF EXISTS "Allow insert access to email notification types" ON email_notification_types;
DROP POLICY IF EXISTS "Allow update access to email notification types" ON email_notification_types;
DROP POLICY IF EXISTS "Allow delete access to email notification types" ON email_notification_types;
DROP POLICY IF EXISTS "Allow insert access to email templates" ON email_templates;
DROP POLICY IF EXISTS "Allow update access to email templates" ON email_templates;
DROP POLICY IF EXISTS "Allow delete access to email templates" ON email_templates;
DROP POLICY IF EXISTS "Allow insert access to email settings" ON email_settings;
DROP POLICY IF EXISTS "Allow update access to email settings" ON email_settings;
DROP POLICY IF EXISTS "Allow delete access to email settings" ON email_settings;
DROP POLICY IF EXISTS "Allow insert access to email logs" ON email_logs;
DROP POLICY IF EXISTS "Allow update access to email logs" ON email_logs;
DROP POLICY IF EXISTS "Allow delete access to email logs" ON email_logs;
...
-- 2. حذف البيانات القديمة ورفع القوالب الأساسية
...
-- 3. إدراج القوالب الأساسية المستخدمة فعلياً
...
-- 4. إدراج أنواع الإشعارات البريدية المستخدمة فعلياً
...
SELECT 
    ...
    COUNT(ent.id) as notification_types_count
FROM email_templates et
LEFT JOIN email_notification_types ent ON ent.template_id = et.id
GROUP BY et.id, et.name, et.name_ar, et.name_en, et.is_active
