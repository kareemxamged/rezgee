-- تشغيل جميع القوالب المفقودة (نسخة مبسطة)
-- بدون استخدام ON CONFLICT لتجنب مشاكل القيود
...
Hurry to reply and start the appropriate conversation! 🌹',
    ...
    true
WHERE NOT EXISTS (
    SELECT 1 FROM email_templates WHERE name = 'message_notification'
);
...
Start communicating with Allah blessing! 🌹',
    ...
    true
WHERE NOT EXISTS (
    SELECT 1 FROM email_templates WHERE name = 'match_notification'
);
...
SELECT 
    ...
    true
WHERE NOT EXISTS (
    SELECT 1 FROM email_notification_types WHERE name = 'like_notification'
);
...
SELECT 
    ...
    true
WHERE NOT EXISTS (
    SELECT 1 FROM email_notification_types WHERE name = 'message_notification'
);
...
SELECT 
    ...
    true
WHERE NOT EXISTS (
    SELECT 1 FROM email_notification_types WHERE name = 'match_notification'
);
...
-- عرض القوالب المضافة
SELECT 
    'القوالب الاجتماعية تم إضافتها بنجاح!' as result;
