...
async function sendRealEmail(data) {
    try {
        ...
        // إعدادات SMTP الصحيحة - الأولوية للبيانات المرسلة
        ...
        console.log('🔧 إعدادات SMTP المستلمة:', {
            host: smtpConfig.host,
            port: smtpConfig.port,
            user: smtpConfig.auth ? .user,
            fromEmail: smtpConfig.from ? .email,
            fromName: smtpConfig.from ? .name,
            source: data.smtpConfig ? 'Custom SMTP Config' : 'Default Config'
        });
        ...
        console.log('🔧 إعداد transporter مع:', {
            ...
            fromEmail: data.from || smtpConfig.from ? .email || smtpConfig.user,
            fromName: data.fromName || smtpConfig.from ? .name,
            source: data.smtpConfig ? 'Custom SMTP Config' : 'Default Config'
        });
        ...
        console.log(`🔧 Source: ${data.smtpConfig ? 'Custom SMTP Config' : 'Default Config'}`);
