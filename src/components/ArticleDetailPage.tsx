...
const ArticleDetailPage: React.FC = () => {
  ...
  // Check if article language matches current language, find similar article if not
  useEffect(() => {
    const handleLanguageChange = async () => {
      if (article && article.language !== currentLanguage) {
        ...
        try {
          ...
          if (similarArticle) {
            // Navigate to the similar article
            navigate(`/articles/${similarArticle.id}`, { replace: true });
          } else {
            // No similar article found, redirect to articles page
            navigate('/articles', {
              replace: true,
              state: {
                message: currentLanguage === 'ar'
                  ? 'لم نجد مقالاً مشابهاً باللغة العربية، تم عرض جميع المقالات المتاحة'
                  : 'No similar article found in English, showing all available articles'
              }
            });
          }
        } catch (error) {
  ...
  }, [article, currentLanguage, navigate]);
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600">
            {languageLoading
              ? (currentLanguage === 'ar' ? 'جاري البحث عن مقال مشابه...' : 'Looking for similar article...')
              : t('articles.loading')
            }
          </p>
        </div>
