...
const PublicProfilePage: React.FC = () => {
  ...
  const checkBlockStatus = async (currentUserId: string, targetUserId: string) => {
    try {
      ...
      // التحقق من حالة الحظر باستخدام استعلام مباشر
  ...
  const handleBlockUser = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // إعادة توجيه المستخدم بعد الحظر (وليس فك الحظر)
        if (!isUserBlocked) {
          setTimeout(() => {
            navigate(-1);
          }, 2000);
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6 mb-6 sm:mb-8 relative">
              ...
              <div className="flex-1 text-center sm:text-right w-full">
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
                  ...
                  {profile.verified && (
                    <Shield className="w-5 h-5 sm:w-6 sm:h-6 text-emerald-600 mx-auto sm:mx-0 flex-shrink-0" />
                  )}
                </div>
