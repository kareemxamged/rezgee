...
interface AcceptReportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAccept: (data: {
    reason: string;
...
const AcceptReportModal: React.FC<AcceptReportModalProps> = ({
  isOpen,
  onClose,
  onAccept,
  reportId,
  ...
  const validateForm = () => {
    ...
    if (!reason.trim()) {
      newErrors.reason = 'سبب القبول مطلوب';
    }

    if (!actionType.trim()) {
      newErrors.actionType = 'نوع الإجراء مطلوب';
    }
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full my-8 max-h-[calc(100vh-4rem)] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-green-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">قبول البلاغ</h2>
        ...
        <div className="p-6 flex-1 overflow-y-auto space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              سبب قبول البلاغ <span className="text-red-500">*</span>
            </label>
            <textarea
              value={reason}
              onChange={(e) => setReason(e.target.value)}
              placeholder="اذكر سبب قبول هذا البلاغ..."
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              نوع الإجراء المتخذ <span className="text-red-500">*</span>
            </label>
            <select
              value={actionType}
            ...
            >
              ...
              <option value="warning">تحذير المستخدم</option>
              <option value="temporary_ban">حظر مؤقت</option>
              <option value="permanent_ban">حظر دائم</option>
              <option value="content_removal">إزالة المحتوى</option>
              <option value="account_restriction">تقييد الحساب</option>
          ...
          <div>
            ...
            >
              ...
              <p className="text-sm text-gray-600 mb-2">
                اسحب الملفات هنا أو 
                <label className="text-green-600 hover:text-green-700 cursor-pointer underline mx-1">
                  تصفح
                  <input
                    type="file"
                    multiple
        ...
        <div className="flex items-center justify-end gap-3 p-6 border-t border-gray-200 bg-gray-50 flex-shrink-0">
          ...
          <button
            ...
            disabled={loading || !reason.trim() || !actionType.trim()}
