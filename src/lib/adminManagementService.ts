      ...
      let query = supabase
        ...
        .select(`
          *,
          ...
          sessions:admin_sessions(count),
          activities:admin_activity_logs(count)
        `, { count: 'exact' });

      // تطبيق الفلاتر
      if (search) {
        query = query.or(`user_profile.email.ilike.%${search}%,user_profile.first_name.ilike.%${search}%,user_profile.last_name.ilike.%${search}%`);
      }
