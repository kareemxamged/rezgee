...
class EmailVerificationService {
  ...
  // دالة لتحويل قيم اللحية من نموذج التسجيل إلى قاعدة البيانات
  private static mapBeardValue(beardValue?: string): string | null {
    ...
    switch (beardValue) {
      ...
      case 'yes':
        return beardValue;
      default:
        console.warn(`⚠️ Unknown beard value: ${beardValue}, setting to null`);
        return null;
    }
  }
  ...
  async sendVerificationEmail(email: string, token: string): Promise<{ success: boolean; error?: string }> {
    try {
      // تم تعطيل إرسال إيميل التحقق "تأكيد إنشاء حسابك في رزجة" بناءً على طلب المستخدم
      console.log('📧 تم تخطي إرسال إيميل التحقق للبريد الإلكتروني:', email);
      
      // إرجاع نجاح وهمي لتجنب كسر التدفق
      return { success: true };
      
      /* 
      // الكود المعطل - إرسال إيميل التحقق
        ...
        body: JSON.stringify({
          ...
          html: `
            <div style="font-family: Arial, sans-serif; direction: rtl; text-align: right;">
              ...
              <a href="${verificationUrl}" style="background: #2563eb; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">تأكيد الحساب</a>
            </div>
          `,
          text: `مرحباً بك في رزقي. يرجى زيارة الرابط التالي لتأكيد حسابك: ${verificationUrl}`,
  ...
  private async createOrUpdateUserProfile(authUser: any, verification: any): Promise<void> {
    ...
    if (existingProfile) {
      ...
      // تم تعطيل إرسال الإيميل الترحيبي - غير مطلوب حالياً
      console.log('ℹ️ تم تخطي إرسال الإيميل الترحيبي (معطل)');
    }
  }
