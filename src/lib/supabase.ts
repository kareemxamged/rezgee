...
// معالج أخطاء Supabase العام (محسن للعمل بصمت)
export const handleSupabaseError = (error: any, context?: string) => {
  ...
  if (errorMessage.includes('Failed to fetch') ||
    ...
    // تسجيل صامت للأخطاء الشبكية
    console.log(`🔄 ${errorContext} مشكلة اتصال مؤقتة، سيتم إعادة المحاولة تلقائياً`);

    // لا نرسل إشعارات للمستخدم في الوضع الصامت
    // النظام سيتعامل مع إعادة المحاولة تلقائياً
    return;
  }

  if (errorMessage.includes('JWT expired') ||
      errorMessage.includes('Invalid token')) {
    console.log(`🔑 ${errorContext} انتهت صلاحية الجلسة، سيتم تجديدها تلقائياً`);

    // محاولة تجديد الجلسة بصمت
    supabase.auth.refreshSession().catch(() => {
      // إذا فشل التجديد، تسجيل خروج صامت
  ...
  // تسجيل الأخطاء الأخرى بصمت (بدون إزعاج المستخدم)
  console.log(`⚠️ ${errorContext} خطأ Supabase:`, error);
};
